...
typedef struct iree_hal_keti_hw1_channel_t {
...
static const iree_hal_channel_vtable_t iree_hal_keti_hw1_channel_vtable;

static iree_hal_keti_hw1_channel_t* iree_hal_keti_hw1_channel_cast(
    iree_hal_channel_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_keti_hw1_channel_vtable);
  return (iree_hal_keti_hw1_channel_t*)base_value;
}

static const iree_hal_keti_hw1_channel_t* iree_hal_keti_hw1_channel_const_cast(
    const iree_hal_channel_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_keti_hw1_channel_vtable);
  return (const iree_hal_keti_hw1_channel_t*)base_value;
}
...
iree_status_t iree_hal_keti_hw1_channel_create(iree_hal_channel_params_t params,
  ...
  *out_channel = NULL;
  ...
  iree_hal_keti_hw1_channel_t* channel = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*channel),
                                (void**)&channel));
  iree_hal_resource_initialize(&iree_hal_keti_hw1_channel_vtable,
                               &channel->resource);
  channel->host_allocator = host_allocator;

  // TODO(keti_hw1): implement channel setup using params. Note that the id is not
...
static void iree_hal_keti_hw1_channel_destroy(iree_hal_channel_t* base_channel) {
  iree_hal_keti_hw1_channel_t* channel = iree_hal_keti_hw1_channel_cast(base_channel);
  IREE_TRACE_ZONE_BEGIN(z0);

  iree_allocator_t host_allocator = channel->host_allocator;

  // TODO(keti_hw1): destroy any implementation resources.
...
static iree_status_t iree_hal_keti_hw1_channel_split(
  ...
  // TODO(keti_hw1): split the channel and get any native resources required.
  ...
  iree_hal_keti_hw1_channel_t* split_channel = NULL;
  if (iree_status_is_ok(status)) {
    status =
  ...
  if (iree_status_is_ok(status)) {
    ...
    // TODO(keti_hw1): transfer ownership of the implementation resources.
  }
...
static void iree_hal_keti_hw1_channel_query_rank_and_count(
    const iree_hal_channel_t* base_channel, int32_t* out_rank,
    int32_t* out_count) {
  const iree_hal_keti_hw1_channel_t* channel =
      iree_hal_keti_hw1_channel_const_cast(base_channel);

  // TODO(keti_hw1): query the rank and count from the implementation or cache them
...
static const iree_hal_channel_vtable_t iree_hal_keti_hw1_channel_vtable = {
    .destroy = iree_hal_keti_hw1_channel_destroy,
    .split = iree_hal_keti_hw1_channel_split,
    .query_rank_and_count = iree_hal_keti_hw1_channel_query_rank_and_count,
};
