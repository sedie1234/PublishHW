...
//===----------------------------------------------------------------------===//
// iree_hal_keti_hw1_semaphore_t
//===----------------------------------------------------------------------===//

typedef struct iree_hal_keti_hw1_semaphore_t {
  iree_hal_semaphore_t base;
  iree_allocator_t host_allocator;
} iree_hal_keti_hw1_semaphore_t;

static const iree_hal_semaphore_vtable_t iree_hal_keti_hw1_semaphore_vtable;

static iree_hal_keti_hw1_semaphore_t* iree_hal_keti_hw1_semaphore_cast(
    iree_hal_semaphore_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_keti_hw1_semaphore_vtable);
  return (iree_hal_keti_hw1_semaphore_t*)base_value;
}

iree_status_t iree_hal_keti_hw1_semaphore_create(
  ...
  *out_semaphore = NULL;
  ...
  iree_hal_keti_hw1_semaphore_t* semaphore = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*semaphore),
                                (void**)&semaphore));
  iree_hal_semaphore_initialize(&iree_hal_keti_hw1_semaphore_vtable,
                                &semaphore->base);
  semaphore->host_allocator = host_allocator;

  // TODO(keti_hw1): implement semaphores. Note that there is some basic support
...
static void iree_hal_keti_hw1_semaphore_destroy(
    iree_hal_semaphore_t* base_semaphore) {
  iree_hal_keti_hw1_semaphore_t* semaphore =
      iree_hal_keti_hw1_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_keti_hw1_semaphore_query(
    iree_hal_semaphore_t* base_semaphore, uint64_t* out_value) {
  *out_value = 0;
  iree_hal_keti_hw1_semaphore_t* semaphore =
      iree_hal_keti_hw1_semaphore_cast(base_semaphore);

  // TODO(keti_hw1): return the current value of the semaphore by (depending on the
  // implementation) making a syscall to get it. It's expected that the value
  // may immediately change after being queried here.

  // TODO(keti_hw1): if the value is IREE_HAL_SEMAPHORE_FAILURE_VALUE then return
...
static iree_status_t iree_hal_keti_hw1_semaphore_signal(
    iree_hal_semaphore_t* base_semaphore, uint64_t new_value) {
  iree_hal_keti_hw1_semaphore_t* semaphore =
      iree_hal_keti_hw1_semaphore_cast(base_semaphore);

  // TODO(keti_hw1): validation is optional but encouraged if cheap: semaphores
  // must always be signaled to a value that is greater than the previous value
  // (not less-than-or-equal).

  // TODO(keti_hw1): signals when the semaphore have failed should also fail and
...
static void iree_hal_keti_hw1_semaphore_fail(iree_hal_semaphore_t* base_semaphore,
                                         iree_status_t status) {
  iree_hal_keti_hw1_semaphore_t* semaphore =
      iree_hal_keti_hw1_semaphore_cast(base_semaphore);
  const iree_status_code_t status_code = iree_status_code(status);

  // TODO(keti_hw1): if the semaphore has already failed and has a status set then
  ...
  // TODO(keti_hw1): set the value to `IREE_HAL_SEMAPHORE_FAILURE_VALUE` as expected
  // by the API.

  // TODO(keti_hw1): take ownership of the status (no need to clone, the caller is
...
static iree_status_t iree_hal_keti_hw1_semaphore_wait(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_timeout_t timeout, iree_hal_wait_flags_t flags) {
  iree_hal_keti_hw1_semaphore_t* semaphore =
      iree_hal_keti_hw1_semaphore_cast(base_semaphore);

  // TODO(keti_hw1): if a failure status is set return
  ...
  // TODO(keti_hw1): prefer having a fast-path for if the semaphore is
...
static iree_status_t iree_hal_keti_hw1_semaphore_import_timepoint(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
...
static iree_status_t iree_hal_keti_hw1_semaphore_export_timepoint(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_hal_queue_affinity_t queue_affinity,
...
static const iree_hal_semaphore_vtable_t iree_hal_keti_hw1_semaphore_vtable = {
    .destroy = iree_hal_keti_hw1_semaphore_destroy,
    .query = iree_hal_keti_hw1_semaphore_query,
    .signal = iree_hal_keti_hw1_semaphore_signal,
    .fail = iree_hal_keti_hw1_semaphore_fail,
    .wait = iree_hal_keti_hw1_semaphore_wait,
    .import_timepoint = iree_hal_keti_hw1_semaphore_import_timepoint,
    .export_timepoint = iree_hal_keti_hw1_semaphore_export_timepoint,
};
