...
//===----------------------------------------------------------------------===//
// iree_hal_keti_hw1_allocator_t
//===----------------------------------------------------------------------===//

// TODO(keti_hw1): use one ID per address space or pool - each shows as a different
// track in tracing tools.
#if IREE_TRACING_FEATURES & IREE_TRACING_FEATURE_ALLOCATION_TRACKING
static const char* IREE_HAL_KETI_HW1_ALLOCATOR_ID = "keti_hw1 unpooled";
#endif  // IREE_TRACING_FEATURE_ALLOCATION_TRACKING

typedef struct iree_hal_keti_hw1_allocator_t {
...
static const iree_hal_allocator_vtable_t iree_hal_keti_hw1_allocator_vtable;

static iree_hal_keti_hw1_allocator_t* iree_hal_keti_hw1_allocator_cast(
    iree_hal_allocator_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_keti_hw1_allocator_vtable);
  return (iree_hal_keti_hw1_allocator_t*)base_value;
}

iree_status_t iree_hal_keti_hw1_allocator_create(
    iree_allocator_t host_allocator, iree_hal_allocator_t** out_allocator) {
  IREE_ASSERT_ARGUMENT(out_allocator);
  IREE_TRACE_ZONE_BEGIN(z0);
  *out_allocator = NULL;

  iree_hal_keti_hw1_allocator_t* allocator = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*allocator),
                                (void**)&allocator));
  iree_hal_resource_initialize(&iree_hal_keti_hw1_allocator_vtable,
                               &allocator->resource);
  allocator->host_allocator = host_allocator;

  // TODO(keti_hw1): query device heaps, supported features (concurrent access/etc),
...
static void iree_hal_keti_hw1_allocator_destroy(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  IREE_ASSERT_ARGUMENT(base_allocator);
  iree_hal_keti_hw1_allocator_t* allocator =
      iree_hal_keti_hw1_allocator_cast(base_allocator);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_allocator_t iree_hal_keti_hw1_allocator_host_allocator(
    const iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  iree_hal_keti_hw1_allocator_t* allocator =
      (iree_hal_keti_hw1_allocator_t*)base_allocator;
  return allocator->host_allocator;
}

static iree_status_t iree_hal_keti_hw1_allocator_trim(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  iree_hal_keti_hw1_allocator_t* allocator =
      (iree_hal_keti_hw1_allocator_t*)base_allocator;

  // TODO(keti_hw1): if the allocator is retaining any unused resources they should
...
static void iree_hal_keti_hw1_allocator_query_statistics(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  IREE_STATISTICS({
    ...
    // TODO(keti_hw1): update statistics (merge).
  });
}

static iree_status_t iree_hal_keti_hw1_allocator_query_memory_heaps(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  // TODO(keti_hw1): return heap information. This is called at least once with a
...
iree_hal_keti_hw1_allocator_query_buffer_compatibility(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  // TODO(keti_hw1): set compatibility rules based on the implementation.
...
static iree_status_t iree_hal_keti_hw1_allocator_allocate_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  // TODO(keti_hw1): allocate the underlying device memory. The impl_ptr is just
  ...
  // is live. An example iree_hal_keti_hw1_buffer_wrap is provided that can be used
  ...
  if (iree_status_is_ok(status)) {
    // TODO(keti_hw1): ensure this accounting is balanced in deallocate_buffer.
    IREE_TRACE_ALLOC_NAMED(IREE_HAL_KETI_HW1_ALLOCATOR_ID, impl_ptr,
                           allocation_size);
...
static void iree_hal_keti_hw1_allocator_deallocate_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
    iree_hal_buffer_t* IREE_RESTRICT base_buffer) {
  iree_hal_keti_hw1_allocator_t* allocator =
      iree_hal_keti_hw1_allocator_cast(base_allocator);

  // TODO(keti_hw1): free the underlying device memory here. Buffers allocated from
  ...
  // TODO(keti_hw1): if the buffer was imported then this accounting may need to be
  // conditional depending on the implementation.
  bool was_imported = false;
  if (!was_imported) {
    IREE_TRACE_FREE_NAMED(IREE_HAL_KETI_HW1_ALLOCATOR_ID, impl_ptr);
...
static iree_status_t iree_hal_keti_hw1_allocator_import_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
    const iree_hal_buffer_params_t* IREE_RESTRICT params,
  ...
  // TODO(keti_hw1): switch on external_buffer->type and import the buffer. See the
...
static iree_status_t iree_hal_keti_hw1_allocator_export_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  // TODO(keti_hw1): switch on requested_type and export as appropriate. Most
...
static const iree_hal_allocator_vtable_t iree_hal_keti_hw1_allocator_vtable = {
    .destroy = iree_hal_keti_hw1_allocator_destroy,
    .host_allocator = iree_hal_keti_hw1_allocator_host_allocator,
    .trim = iree_hal_keti_hw1_allocator_trim,
    .query_statistics = iree_hal_keti_hw1_allocator_query_statistics,
    .query_memory_heaps = iree_hal_keti_hw1_allocator_query_memory_heaps,
    .query_buffer_compatibility =
        iree_hal_keti_hw1_allocator_query_buffer_compatibility,
    .allocate_buffer = iree_hal_keti_hw1_allocator_allocate_buffer,
    .deallocate_buffer = iree_hal_keti_hw1_allocator_deallocate_buffer,
    .import_buffer = iree_hal_keti_hw1_allocator_import_buffer,
    .export_buffer = iree_hal_keti_hw1_allocator_export_buffer,
};
