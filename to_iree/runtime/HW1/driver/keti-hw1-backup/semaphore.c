...
typedef struct iree_halhw1semaphore_t {
  iree_hal_semaphore_t base;
  iree_allocator_t host_allocator;
} iree_halhw1semaphore_t;

static const iree_hal_semaphore_vtable_t iree_halhw1semaphore_vtable;

static iree_halhw1semaphore_t* iree_halhw1semaphore_cast(
    iree_hal_semaphore_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_halhw1semaphore_vtable);
  return (iree_halhw1semaphore_t*)base_value;
}
...
iree_status_t iree_halhw1semaphore_create(
  ...
  *out_semaphore = NULL;
  ...
  iree_halhw1semaphore_t* semaphore = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*semaphore),
                                (void**)&semaphore));
  iree_hal_semaphore_initialize(&iree_halhw1semaphore_vtable,
                                &semaphore->base);
...
static void iree_halhw1semaphore_destroy(
    iree_hal_semaphore_t* base_semaphore) {
  iree_halhw1semaphore_t* semaphore =
      iree_halhw1semaphore_cast(base_semaphore);
...
static iree_status_t iree_halhw1semaphore_query(
...
static iree_status_t iree_halhw1semaphore_signal(
...
static void iree_halhw1semaphore_fail(iree_hal_semaphore_t* base_semaphore,
                                         iree_status_t status) {
  iree_halhw1semaphore_t* semaphore =
      iree_halhw1semaphore_cast(base_semaphore);
...
static iree_status_t iree_halhw1semaphore_wait(
...
static iree_status_t iree_halhw1semaphore_import_timepoint(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
...
static iree_status_t iree_halhw1semaphore_export_timepoint(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_hal_queue_affinity_t queue_affinity,
...
static const iree_hal_semaphore_vtable_t iree_halhw1semaphore_vtable = {
    .destroy = iree_halhw1semaphore_destroy,
    .query = iree_halhw1semaphore_query,
    .signal = iree_halhw1semaphore_signal,
    .fail = iree_halhw1semaphore_fail,
    .wait = iree_halhw1semaphore_wait,
    .import_timepoint = iree_halhw1semaphore_import_timepoint,
    .export_timepoint = iree_halhw1semaphore_export_timepoint,
};
