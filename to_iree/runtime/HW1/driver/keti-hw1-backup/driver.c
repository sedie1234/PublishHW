...
//===----------------------------------------------------------------------===//
// iree_halhw1driver_options_t
//===----------------------------------------------------------------------===//

IREE_API_EXPORT void iree_halhw1driver_options_initialize(
    iree_halhw1driver_options_t* out_options) {
  ...
  iree_halhw1device_options_initialize(&out_options->default_device_options);
}

static iree_status_t iree_halhw1driver_options_verify(
    const iree_halhw1driver_options_t* options) {
...
// only device.
#define IREE_HALHW1DEVICE_ID_DEFAULT 0

typedef struct iree_halhw1driver_t {
...
static const iree_hal_driver_vtable_t iree_halhw1driver_vtable;

static iree_halhw1driver_t* iree_halhw1driver_cast(
    iree_hal_driver_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_halhw1driver_vtable);
  return (iree_halhw1driver_t*)base_value;
}

IREE_API_EXPORT iree_status_t iree_halhw1driver_create(
    iree_string_view_t identifier,
    const iree_halhw1driver_options_t* options,
  ...
  *out_driver = NULL;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_halhw1driver_options_verify(options));

  iree_halhw1driver_t* driver = NULL;
  iree_host_size_t total_size = sizeof(*driver) + identifier.size;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, total_size, (void**)&driver));
  iree_hal_resource_initialize(&iree_halhw1driver_vtable, &driver->resource);
...
static void iree_halhw1driver_destroy(iree_hal_driver_t* base_driver) {
  iree_halhw1driver_t* driver = iree_halhw1driver_cast(base_driver);
...
static iree_status_t iree_halhw1driver_query_available_devices(
  ...
  static const iree_hal_device_info_t device_infos[1] = {
      {
          .device_id = IREE_HALHW1DEVICE_ID_DEFAULT,
...
static iree_status_t iree_halhw1driver_dump_device_info(
...
static iree_status_t iree_halhw1driver_create_device_by_id(
    iree_hal_driver_t* base_driver, iree_hal_device_id_t device_id,
  ...
  iree_halhw1device_options_t options =
  ...
  return iree_halhw1device_create(driver->identifier, &options,
                                     host_allocator, out_device);
}

static iree_status_t iree_halhw1driver_create_device_by_path(
...
static const iree_hal_driver_vtable_t iree_halhw1driver_vtable = {
    .destroy = iree_halhw1driver_destroy,
    .query_available_devices = iree_halhw1driver_query_available_devices,
    .dump_device_info = iree_halhw1driver_dump_device_info,
    .create_device_by_id = iree_halhw1driver_create_device_by_id,
    .create_device_by_path = iree_halhw1driver_create_device_by_path,
};
