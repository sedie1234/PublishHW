...
//===----------------------------------------------------------------------===//
// iree_halhw1channel_t
//===----------------------------------------------------------------------===//

typedef struct iree_halhw1channel_t {
...
static const iree_hal_channel_vtable_t iree_halhw1channel_vtable;

static iree_halhw1channel_t* iree_halhw1channel_cast(
    iree_hal_channel_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_halhw1channel_vtable);
  return (iree_halhw1channel_t*)base_value;
}

static const iree_halhw1channel_t* iree_halhw1channel_const_cast(
    const iree_hal_channel_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_halhw1channel_vtable);
  return (const iree_halhw1channel_t*)base_value;
}

iree_status_t iree_halhw1channel_create(iree_hal_channel_params_t params,
                                           iree_allocator_t host_allocator,
  ...
  *out_channel = NULL;
  ...
  iree_halhw1channel_t* channel = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*channel),
                                (void**)&channel));
  iree_hal_resource_initialize(&iree_halhw1channel_vtable,
                               &channel->resource);
...
static void iree_halhw1channel_destroy(iree_hal_channel_t* base_channel) {
  iree_halhw1channel_t* channel = iree_halhw1channel_cast(base_channel);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_status_t iree_halhw1channel_split(
  ...
  iree_halhw1channel_t* split_channel = NULL;
  if (iree_status_is_ok(status)) {
    status =
...
static void iree_halhw1channel_query_rank_and_count(
    const iree_hal_channel_t* base_channel, int32_t* out_rank,
    int32_t* out_count) {
  const iree_halhw1channel_t* channel =
      iree_halhw1channel_const_cast(base_channel);
...
static const iree_hal_channel_vtable_t iree_halhw1channel_vtable = {
    .destroy = iree_halhw1channel_destroy,
    .split = iree_halhw1channel_split,
    .query_rank_and_count = iree_halhw1channel_query_rank_and_count,
};
